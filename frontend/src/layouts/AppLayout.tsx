...
export default function AppLayout() {
  ...
  const { role, canEdit, isAdmin, isLead } = useUserRole();
  ...
  const navItems = [
    ...
    { to: '/app/board', icon: Kanban, label: t('nav.board') },
    { to: '/app/table', icon: Table, label: t('nav.table') },
  ];
  ...
  // Панель OnlyAdmin только для супер-админа
  const superAdminItems = isAdmin ? [
    { to: '/app/admin', icon: Shield, label: 'OnlyAdmin' },
  ] : [];
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display font-bold text-slate-800">NNMC IT Board</h1>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 flex items-center gap-1">
                  <span className={`w-2 h-2 rounded-full ${canEdit ? 'bg-primary-500' : 'bg-slate-400'}`} />
                  {t(`role.${role}`)}
                </p>
              </div>
